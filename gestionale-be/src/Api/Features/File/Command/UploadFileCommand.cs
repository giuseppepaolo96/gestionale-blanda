        ...
        public async Task<IResult> ExecuteAsync(UploadFileCommand command, CancellationToken ct)
        {
            ...
            foreach (var file in command.Files)
            {
                ...
                try
                {
                    ...
                    if (fileName.Contains("sponsor", StringComparison.OrdinalIgnoreCase))
                    {
                        var sponsorName = fileName.Replace("Sponsor", "", StringComparison.OrdinalIgnoreCase)
                                                  .Replace(fileExtension, "")
                                                  .Trim();

                        byte[] sponsorData = memoryStream.ToArray(); // Contenuto binario del logo
                        ...
                        {
                            existingSponsor.SponsorLogo = sponsorData;
                        ...
                        successfullyUploadedFiles.Add(fileName);
                        continue; 
                    }


                    // Salva gli altri file nella tabella FileRecords
                    ...
                    {
                        var teamName = fileName.Replace("Logo", "", StringComparison.OrdinalIgnoreCase)
                                                .Replace(fileExtension, "")
                                                .Trim();
                        byte[] teamLogoData = memoryStream.ToArray(); // Usa direttamente i dati binari
                        ...
                        if (existingTeam == null)
                        {
                            ...
                            {
                                Name = teamName,
                                Logo = teamLogoData // Salva direttamente i dati binari
                            };

                            _context.Teams.Add(team);
                            await _context.SaveChangesAsync(ct); // Salva i cambiamenti nel database
                        ...
                        {
                            existingTeam.Logo = teamLogoData; // Aggiorna il logo con i dati binari
